...
const ExperienceTimelineItem: React.FC<{ item: PortfolioItem }> = ({
  item,
}) => (
...
const TechToolCard: React.FC<{ item: PortfolioItem }> = ({ item }) => (
  <div className="group flex h-full flex-col overflow-hidden rounded-none border-2 border-black bg-white p-5 shadow-[4px_4px_0px_#000] transition-all duration-150 hover:-translate-y-0.5 hover:shadow-[6px_6px_0px_#4f46e5]">
    {item.image_url && (
      <div className="mb-3 flex justify-center h-12">
        <img
          src={item.image_url}
          alt={`${item.title} logo`}
                ...
                <>
                  ...
                  {(section.title.toLowerCase().includes("tech") ||
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                      {section.portfolio_items.map((item) => (
                        <TechToolCard key={item.id} item={item} />
                      ))}
                    </div>
                  )}
                  ...
                  {!(
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                      {section.portfolio_items.map((item) => (
                        <div
                          key={item.id}
                          className="rounded-none border-2 border-black bg-gray-50 p-4 shadow-[3px_3px_0_#000]"
                        >
