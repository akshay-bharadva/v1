...
/*
This file is heavily redesigned for the neo-brutalist theme.
- All styling is now handled by the Tailwind Typography plugin (`prose`) which is configured for the new theme.
- The custom code block styling is made starker with thick borders and sharp corners.
- The overall page layout is functional, with a two-column design for article and metadata.
- `PostHeader`, `AuthorInfo`, and `PostTagsSidebar` are restyled to be raw and bold, using neo-brutalist components (`Avatar`, `Badge`).
- The "Not Found" display is redesigned to match the new, stark aesthetic.
*/
...
const CodeBlock = ({ className, children }: { className?: string, children: React.ReactNode }) => {
  ...
  return (
    <div className="relative my-6 rounded-none border-2 border-foreground bg-secondary/50 font-sans">
      <div className="flex items-center justify-between border-b-2 border-foreground px-4 py-1.5 text-xs">
        <span className="font-bold uppercase text-muted-foreground">{lang}</span>
...
const markdownComponents: any = {
  ...
  img: ({ node, ...props }: any) => (
    <img {...props} loading="lazy" className="my-8 h-auto max-h-[80vh] w-full rounded-none border-2 border-foreground object-contain" alt={props.alt || "Blog image"} />
  ),
};

const PostHeader = ({ post }: { post: BlogPost }) => (
  <header className="mb-8">
    <h1 className="mb-4 text-3xl font-black uppercase leading-tight tracking-tighter text-foreground md:text-4xl lg:text-5xl">
      {post.title}
    </h1>
...
const PostTagsSidebar = ({ tags }: { tags: string[] }) => (
  <div>
    <h3 className="mb-3 text-sm font-bold uppercase tracking-wider text-muted-foreground">Tags</h3>
...
const NotFoundDisplay = () => (
    <div className="flex min-h-[calc(100vh-10rem)] items-center justify-center p-4">
        ...
        <div className="w-full max-w-md text-center">
            <h1 className="text-6xl font-black uppercase text-foreground">404</h1>
            <p className="mt-2 text-xl font-bold text-muted-foreground">Post Not Found</p>
...
export default function BlogPostPage() {
  ...
  if (error) { return <Layout><div className="flex min-h-screen items-center justify-center p-4"><div className="rounded-none border-2 border-destructive bg-destructive/10 p-6 font-bold text-destructive">Error: {error}. <Link href="/blog" className="underline hover:text-foreground">Back to blog</Link></div></div></Layout>; }
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mx-auto max-w-5xl px-4">
          <div className="grid grid-cols-1 lg:grid-cols-12 lg:gap-x-12">
            <article className="lg:col-span-9">
              ...
              {post.cover_image_url && <img src={post.cover_image_url} alt={post.title} className="my-8 w-full h-auto rounded-none border-2 border-foreground object-cover" />}
