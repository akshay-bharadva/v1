...
// - It now uses the updated dark-themed <Card>, <Tabs>, and <Table> components.
// - The chart container is updated with a dark background.
// - Text colors for earnings (green) and expenses (red) are adjusted for better visibility on a dark background.
...
const chartConfig = {
    earning: { label: "Earnings", color: "hsl(140 70% 60%)" }, // Brighter green
    expense: { label: "Expenses", color: "hsl(0 70% 60%)" },   // Brighter red
        ...
        <div className="space-y-6 font-sans">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-zinc-400">Track your earnings and expenses.</p>
                </div>
            ...
            <Tabs defaultValue="dashboard">
                ...
                <TabsContent value="dashboard" className="space-y-6">
                    <div className="grid gap-4 md:grid-cols-3">
                        <Card><CardHeader><CardTitle>This Month's Earnings</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold text-green-400">${totalEarnings.toFixed(2)}</p></CardContent></Card>
                        <Card><CardHeader><CardTitle>This Month's Expenses</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold text-red-400">${totalExpenses.toFixed(2)}</p></CardContent></Card>
                        <Card><CardHeader><CardTitle>This Month's Net</CardTitle></CardHeader><CardContent><p className={`text-2xl font-bold ${netSavings >= 0 ? 'text-blue-400' : 'text-orange-400'}`}>${netSavings.toFixed(2)}</p></CardContent></Card>
                    </div>
                    <Card>
                                ...
                                <TableBody>
                                    {transactions.map(t => (
                                        <TableRow key={t.id}>
                                            ...
                                            <TableCell className={`text-right font-bold ${t.type === 'earning' ? 'text-green-400' : 'text-red-400'}`}>{t.type === 'earning' ? '+' : '-'}${t.amount.toFixed(2)}</TableCell>
                                            <TableCell className="flex gap-1"><Button variant="ghost" size="icon" className="size-8" onClick={() => handleEdit(t)}><Edit className="size-4" /></Button><Button variant="ghost" size="icon" className="size-8 hover:bg-red-900/50" onClick={() => handleDelete(t.id)}><Trash2 className="size-4" /></Button></TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
