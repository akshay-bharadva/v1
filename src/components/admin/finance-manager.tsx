...
export default function FinanceManager() {
  ...
  const { totalEarnings, totalExpenses, netSavings, monthlyData } =
    useMemo(() => {
      const now = new Date();
      ...
      const monthlyData = Array.from({ length: 6 })
        .map((_, i) => {
          ...
          const monthTransactions = transactions.filter((t) => {
            const tDate = new Date(t.date);
            return (
              tDate.getMonth() === d.getMonth() && tDate.getFullYear() === year
            );
          });

          return {
            name: `${month} '${String(year).slice(-2)}`,
            earning: monthTransactions
              .filter((t) => t.type === "earning")
              .reduce((acc, t) => acc + t.amount, 0),
        ...
        <TabsContent value="dashboard" className="mt-4 space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent>
                <p
                  className={`text-2xl font-bold ${netSavings >= 0 ? "text-foreground" : "text-destructive"}`}
                >
                ...
                <TableBody>
                  ...
                  {transactions.map((t) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell
                        className={`text-right font-bold ${t.type === "earning" ? "text-accent" : "text-destructive"}`}
                      >
