...
const chartConfig = { earning: { label: "Earnings", color: "#22c55e" }, expense: { label: "Expenses", color: "#ef4444" } };
...
const GoalCard = ({ goal, onAddFunds, onEdit, onDelete }: { goal: FinancialGoal, onAddFunds: () => void, onEdit: () => void, onDelete: () => void }) => {
    ...
    return (
        <Card className="relative flex flex-col overflow-hidden transition-transform hover:-translate-y-1">
            <motion.div
                className="absolute bottom-0 left-0 right-0 bg-yellow-300 -z-0"
                initial={{ height: 0 }}
            ...
            <div className="relative z-10 flex h-full flex-col">
                <CardHeader>
                    <div className="flex items-start justify-between">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="-mr-2 -mt-2 size-7 text-neutral-500"><Edit className="size-4" /></Button></DropdownMenuTrigger>
                            <DropdownMenuContent>
                ...
                <CardContent className="flex flex-grow flex-col justify-between text-center">
                    <div>
                        <p className="text-6xl font-black text-black">{progress.toFixed(0)}<span className="text-4xl text-neutral-400">%</span></p>
                    ...
                    <div className="mt-8 grid grid-cols-2 gap-4 text-sm">
                        <div className="space-y-1 rounded-none border border-black bg-white p-2">
                            <p className="text-xs font-semibold uppercase text-neutral-500">Remaining</p>
                            <p className="text-lg font-bold text-black">
                                ${remainingAmount > 0 ? remainingAmount.toLocaleString() : 0}
                            </p>
                        </div>
                        <div className="space-y-1 rounded-none border border-black bg-white p-2">
                            <p className="text-xs font-semibold uppercase text-neutral-500">Time Left</p>
                            {remainingDays !== null ? (
                                <p className={`text-lg font-bold ${remainingDays < 0 ? 'text-destructive' : 'text-black'}`}>
...
export default function FinanceManager() {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-bold">Finance Command Center</h2><p className="text-neutral-600">Strategic overview, planning, and transaction management.</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => setDialogState({ type: "goal" })}> <Target className="mr-2 size-4" /> New Goal</Button><Button onClick={() => setDialogState({ type: "recurring" })}> <Repeat className="mr-2 size-4" /> New Recurring</Button><Button onClick={() => setDialogState({ type: "transaction" })}> <Plus className="mr-2 size-4" /> New Transaction</Button></div></div>
            <Tabs defaultValue="dashboard">
                <TabsList><TabsTrigger value="dashboard">Dashboard</TabsTrigger><TabsTrigger value="transactions">Transactions</TabsTrigger><TabsTrigger value="recurring">Recurring</TabsTrigger><TabsTrigger value="goals">Goals</TabsTrigger><TabsTrigger value="analytics">Analytics</TabsTrigger></TabsList>
                <TabsContent value="dashboard" className="mt-6 space-y-6">
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                        <div className="lg:col-span-2 space-y-6">
                            <Card>
                                <CardHeader className="border-b-2"><CardTitle>Net Income Overview</CardTitle><CardDescription>Total earnings minus total expenses for: {date?.from ? format(date.from, "LLL dd, y") : '...'} - {date?.to ? format(date.to, "LLL dd, y") : '...'}</CardDescription></CardHeader>
                                <CardContent className="pt-6 space-y-6"><div className={`text-5xl font-bold ${netIncome >= 0 ? 'text-green-600' : 'text-red-600'}`}>{netIncome >= 0 ? '+' : '-'}${Math.abs(netIncome).toFixed(2)}</div><div className="flex justify-around items-center pt-4 border-t-2"><div className="text-center"><p className="text-sm text-neutral-500 flex items-center gap-1"><TrendingUp className="size-4 text-green-500" /> Total Earnings</p><p className="text-2xl font-semibold">${totalEarnings.toFixed(2)}</p></div><Separator orientation="vertical" className="h-12 bg-black" /><div className="text-center"><p className="text-sm text-neutral-500 flex items-center gap-1"><TrendingDown className="size-4 text-red-500" /> Total Expenses</p><p className="text-2xl font-semibold">${totalExpenses.toFixed(2)}</p></div></div></CardContent>
                            </Card>
                            <Card>
                                <CardHeader className="border-b-2"><CardTitle>Cash Flow (Last 6 Months)</CardTitle></CardHeader>
                                <CardContent className="pt-6"><ChartContainer config={chartConfig} className="h-64 w-full"><BarChart data={cashFlowData}><CartesianGrid vertical={false} /><XAxis dataKey="name" tickLine={false} axisLine={false} tickMargin={8} /><YAxis tickLine={false} axisLine={false} tickMargin={8} /><ChartTooltip content={<ChartTooltipContent />} /><ChartLegend content={<ChartLegendContent />} /><Bar dataKey="earning" fill="var(--color-earning)" radius={0} /><Bar dataKey="expense" fill="var(--color-expense)" radius={0} /></BarChart></ChartContainer></CardContent>
                            </Card>
                            <Card>
                                <CardHeader className="border-b-2"><CardTitle>Goals at a Glance</CardTitle><CardDescription>A summary of your active financial targets.</CardDescription></CardHeader>
                                <CardContent className="pt-6 space-y-6">{goals.length > 0 ? goals.map(goal => { const progress = Math.min((goal.current_amount / goal.target_amount) * 100, 100); const remainingAmount = goal.target_amount - goal.current_amount; const remainingDays = goal.target_date ? differenceInDays(new Date(goal.target_date), new Date()) : null; return (<div key={goal.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div className="flex-grow"><p className="font-semibold">{goal.name}</p><p className="text-sm text-neutral-500">${goal.current_amount.toLocaleString()} / ${goal.target_amount.toLocaleString()}</p></div><div className="flex items-center gap-4 w-full sm:w-auto"><TooltipProvider delayDuration={100}><Tooltip><TooltipTrigger asChild><div className="relative h-6 w-28 rounded-none border-2 border-black bg-white overflow-hidden"><motion.div className="absolute left-0 top-0 h-full bg-yellow-300" initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.8, ease: "easeOut" }} /><span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-black">{progress.toFixed(0)}%</span></div></TooltipTrigger>{remainingDays !== null && (<TooltipContent><p>{remainingDays < 0 ? `${Math.abs(remainingDays)} days overdue` : `${remainingDays} days remaining`}</p></TooltipContent>)}</Tooltip></TooltipProvider><div className="text-right w-24"><p className="text-xs text-neutral-500">Remaining</p><p className="font-bold text-base">${remainingAmount > 0 ? remainingAmount.toLocaleString() : 0}</p></div></div></div>)}) : <p className="text-neutral-500 text-sm py-4 text-center">No goals set. Create one in the 'Goals' tab to see progress here.</p>}</CardContent>
                                {goals.length > 0 && (<CardFooter><p className="text-xs text-neutral-500">Manage your goals in the 'Goals' tab.</p></CardFooter>)}
                            </Card>
                        </div>
                        <div className="lg:col-span-1 space-y-6">
                            <Card>
                                <CardHeader className="border-b-2"><CardTitle>Expense Breakdown</CardTitle></CardHeader>
                                <CardContent className="pt-6">{expenseByCategory.length > 0 ? <ChartContainer config={{}} className="h-64 w-full"><PieChart><Pie data={expenseByCategory} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={80}>{expenseByCategory.map((_, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie><ChartTooltip content={<ChartTooltipContent nameKey="name" />} /></PieChart></ChartContainer> : <div className="flex h-64 items-center justify-center"><p className="text-neutral-500 text-center">No expense data for this period.</p></div>}</CardContent>
                            </Card>
                            <Card>
                                <CardHeader className="border-b-2"><CardTitle>Upcoming in 30 Days</CardTitle></CardHeader>
                                <CardContent className="pt-6 space-y-3">{forecast.upcomingTransactions.length > 0 ? forecast.upcomingTransactions.map((t, i) => (<div key={i} className="flex justify-between items-center text-sm"><div><p className="font-semibold">{t.description}</p><p className="text-xs text-neutral-500">Due in {differenceInDays(t.date, new Date())} days ({format(t.date, 'MMM dd')})</p></div><p className={`font-bold ${t.type === 'earning' ? 'text-green-600' : 'text-red-600'}`}>{t.type === 'earning' ? '+' : '-'}${t.amount.toFixed(2)}</p></div>)) : <p className="text-sm text-neutral-500 text-center py-8">No upcoming recurring transactions.</p>}</CardContent>
                            </Card>
                        </div>
                    </div>
                </TabsContent>
                <TabsContent value="transactions" className="mt-6 space-y-4">
                    <Card><CardContent className="pt-6 flex flex-col gap-4 sm:flex-row sm:items-center"><div className="relative flex-grow"><Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-neutral-500" /><Input placeholder="Search descriptions..." className="pl-10" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div><Popover><PopoverTrigger asChild><Button variant="outline" className="w-full justify-start text-left font-normal sm:w-auto"><CalendarIcon className="mr-2 size-4" />{date?.from ? (date.to ? `${format(date.from, "LLL dd, y")} - ${format(date.to, "LLL dd, y")}` : format(date.from, "LLL dd, y")) : <span>Pick a date range</span>}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar initialFocus mode="range" defaultMonth={date?.from} selected={date} onSelect={setDate} numberOfMonths={2} /></PopoverContent></Popover></CardContent></Card>
                    <Table><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Description</TableHead><TableHead>Category</TableHead><TableHead className="text-right">Amount</TableHead><TableHead className="w-[100px] text-center">Actions</TableHead></TableRow></TableHeader><TableBody>{filteredTransactions.length > 0 ? (filteredTransactions.map((t) => (<TableRow key={t.id}><TableCell>{format(new Date(t.date + 'T00:00:00'), "MMM dd, yyyy")}</TableCell><TableCell className="font-medium">{t.description}</TableCell><TableCell>{t.category || "–"}</TableCell><TableCell className={`text-right font-bold ${t.type === "earning" ? "text-green-600" : "text-red-600"}`}>{t.type === "earning" ? "+" : "-"}${t.amount.toFixed(2)}</TableCell><TableCell className="text-center"><Button variant="ghost" size="icon" className="size-8" onClick={() => setDialogState({ type: "transaction", data: t })}><Edit className="size-4" /></Button><Button variant="ghost" size="icon" className="size-8 text-neutral-500 hover:bg-red-100 hover:text-destructive" onClick={() => handleDelete("transactions", t.id, "Are you sure you want to delete this transaction?")}><Trash2 className="size-4" /></Button></TableCell></TableRow>))) : (<TableRow><TableCell colSpan={5} className="h-24 text-center">No transactions found for the selected filters.</TableCell></TableRow>)}</TableBody></Table>
                </TabsContent>
                <TabsContent value="recurring">
                    <Card><CardHeader className="border-b-2"><CardTitle>Recurring Transactions</CardTitle><CardDescription>Automate your regular income and expenses to forecast cash flow. Due transactions are logged automatically.</CardDescription></CardHeader><CardContent className="pt-6"><Table><TableHeader><TableRow><TableHead>Description</TableHead><TableHead>Amount</TableHead><TableHead>Schedule</TableHead><TableHead>Next Due</TableHead><TableHead className="text-center">Actions</TableHead></TableRow></TableHeader><TableBody>{recurring.map((r) => { let schedule: string = r.frequency; if ((r.frequency === 'weekly' || r.frequency === 'bi-weekly') && r.occurrence_day !== null && r.occurrence_day !== undefined) { const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']; schedule = `${r.frequency} on ${days[r.occurrence_day]}`; } else if (r.frequency === 'monthly' && r.occurrence_day) { schedule = `monthly on the ${r.occurrence_day}th`; } let nextDueDate = 'N/A'; try { let cursor = r.last_processed_date ? new Date(r.last_processed_date + 'T00:00:00') : new Date(r.start_date + 'T00:00:00'); if (isBefore(cursor, new Date(r.start_date + 'T00:00:00'))) cursor = new Date(r.start_date + 'T00:00:00'); const next = (r.last_processed_date && isAfter(new Date(r.last_processed_date + 'T00:00:00'), new Date())) ? cursor : getNextOccurrence(cursor, r); nextDueDate = format(next, 'MMM dd, yyyy'); } catch (e) { console.error("Date calculation error", e); } return (<TableRow key={r.id}><TableCell className="font-medium">{r.description}</TableCell><TableCell className={r.type === 'earning' ? 'text-green-600' : 'text-red-600'}>${r.amount.toFixed(2)}</TableCell><TableCell className="capitalize">{schedule}</TableCell><TableCell>{nextDueDate}</TableCell><TableCell className="text-center"><Button variant="ghost" size="icon" className="size-8" onClick={() => setDialogState({ type: 'recurring', data: r })}><Edit className="size-4" /></Button><Button variant="ghost" size="icon" className="size-8 text-neutral-500 hover:bg-red-100 hover:text-destructive" onClick={() => handleDelete("recurring_transactions", r.id, `Delete rule "${r.description}"?`)}><Trash2 className="size-4" /></Button></TableCell></TableRow>) })} {recurring.length === 0 && <TableRow><TableCell colSpan={5} className="h-24 text-center">No recurring transaction rules found.</TableCell></TableRow>}</TableBody></Table></CardContent></Card>
                </TabsContent>
                <TabsContent value="goals">
                    <Card><CardHeader className="border-b-2"><CardTitle>Financial Goals</CardTitle><CardDescription>Set targets and track your progress towards them.</CardDescription></CardHeader><CardContent className="pt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{goals.map((goal) => (<GoalCard key={goal.id} goal={goal} onAddFunds={() => setDialogState({ type: 'addFunds', data: goal })} onEdit={() => setDialogState({ type: 'goal', data: goal })} onDelete={() => handleDelete("financial_goals", goal.id, `Delete goal "${goal.name}"?`)} />))} {goals.length === 0 && <p className="text-neutral-500 col-span-full text-center py-12">No goals set yet. Click "New Goal" to start planning.</p>}</CardContent></Card>
                </TabsContent>
                <TabsContent value="analytics">
                    <Card><CardHeader className="border-b-2 flex-row items-center justify-between"><div><CardTitle>Annual Report</CardTitle><CardDescription>Month-by-month financial summary for the selected year.</CardDescription></div><Select value={String(analyticsYear)} onValueChange={(v) => setAnalyticsYear(Number(v))}><SelectTrigger className="w-[120px]"><SelectValue placeholder="Select Year" /></SelectTrigger><SelectContent>{Array.from({ length: 5 }, (_, i) => new Date().getFullYear() - i).map(year => (<SelectItem key={year} value={String(year)}>{year}</SelectItem>))}</SelectContent></Select></CardHeader><CardContent className="pt-6"><Table><TableHeader><TableRow><TableHead>Month</TableHead><TableHead className="text-right text-green-600">Income</TableHead><TableHead className="text-right text-red-600">Expenses</TableHead><TableHead className="text-right">Net Income</TableHead></TableRow></TableHeader><TableBody>{analyticsData.map(row => (<TableRow key={row.month}><TableCell className="font-medium">{row.month}</TableCell><TableCell className="text-right text-green-600">+${row.income.toFixed(2)}</TableCell><TableCell className="text-right text-red-600">-${row.expenses.toFixed(2)}</TableCell><TableCell className={`text-right font-bold ${row.net >= 0 ? 'text-black' : 'text-destructive'}`}>{row.net < 0 ? '-' : ''}${Math.abs(row.net).toFixed(2)}</TableCell></TableRow>))}</TableBody></Table></CardContent></Card>
                </TabsContent>
            </Tabs>
