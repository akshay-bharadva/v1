...
const GoalCard = ({ goal, onAddFunds, onEdit, onDelete }: { goal: FinancialGoal, onAddFunds: () => void, onEdit: () => void, onDelete: () => void }) => {
    ...
    return (
        <Card className="relative flex flex-col overflow-hidden transition-all duration-300 hover:shadow-none active:translate-x-1 active:translate-y-1">
            <motion.div
                className="absolute bottom-0 left-0 right-0 bg-accent/20 -z-0"
                initial={{ height: 0 }}
            ...
            <div className="relative z-10 flex h-full flex-col">
                ...
                <CardContent className="flex flex-grow flex-col justify-between text-center">
                    <div>
                        <p className="text-6xl font-black text-accent">{progress.toFixed(0)}<span className="text-4xl text-accent/50">%</span></p>
                    ...
                    <div className="mt-8 grid grid-cols-2 gap-4 text-sm">
                        <div className="space-y-1 rounded-none border-2 border-foreground bg-background/50 p-2">
                        ...
                        <div className="space-y-1 rounded-none border-2 border-foreground bg-background/50 p-2">
                            <p className="text-xs font-bold uppercase text-muted-foreground">Time Left</p>
                            {remainingDays !== null ? (
...
export default function FinanceManager() {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-black uppercase">Finance Manager</h2><p className="text-muted-foreground">Strategic overview and transaction management.</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => setDialogState({ type: "goal" })}> <Target className="mr-2 size-4" /> New Goal</Button><Button onClick={() => setDialogState({ type: "recurring" })}> <Repeat className="mr-2 size-4" /> New Recurring</Button><Button onClick={() => setDialogState({ type: "transaction" })}> <Plus className="mr-2 size-4" /> New Transaction</Button></div></div>
                ...
                <TabsContent value="dashboard" className="mt-6 space-y-6">
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                        <div className="lg:col-span-2 space-y-6">
                            <Card>
                                <CardHeader><CardTitle>Net Income Overview</CardTitle><CardDescription>Earnings vs expenses for: {date?.from ? format(date.from, "LLL dd, y") : '...'} - {date?.to ? format(date.to, "LLL dd, y") : '...'}</CardDescription></CardHeader>
                                <CardContent className="space-y-6"><div className={`text-5xl font-black ${netIncome >= 0 ? 'text-green-600' : 'text-red-600'}`}>{netIncome >= 0 ? '+' : '-'}${Math.abs(netIncome).toFixed(2)}</div><div className="flex justify-around items-center pt-4 border-t-2 border-foreground"><div className="text-center"><p className="text-sm text-muted-foreground flex items-center gap-1"><TrendingUp className="size-4 text-green-500" /> Total Earnings</p><p className="text-2xl font-bold">${totalEarnings.toFixed(2)}</p></div><Separator orientation="vertical" className="h-12" /><div className="text-center"><p className="text-sm text-muted-foreground flex items-center gap-1"><TrendingDown className="size-4 text-red-500" /> Total Expenses</p><p className="text-2xl font-bold">${totalExpenses.toFixed(2)}</p></div></div></CardContent>
                            </Card>
                            <Card>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-6">{goals.length > 0 ? goals.map(goal => { const progress = Math.min((goal.current_amount / goal.target_amount) * 100, 100); const remainingAmount = goal.target_amount - goal.current_amount; const remainingDays = goal.target_date ? differenceInDays(new Date(goal.target_date), new Date()) : null; return (<div key={goal.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div className="flex-grow"><p className="font-bold">{goal.name}</p><p className="text-sm text-muted-foreground">${goal.current_amount.toLocaleString()} / ${goal.target_amount.toLocaleString()}</p></div><div className="flex items-center gap-4 w-full sm:w-auto"><TooltipProvider delayDuration={100}><Tooltip><TooltipTrigger asChild><div className="relative h-6 w-28 rounded-none bg-muted overflow-hidden border-2 border-foreground"><motion.div className="absolute left-0 top-0 h-full rounded-none bg-accent" initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.8, ease: "easeOut" }} /><span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-accent-foreground">{progress.toFixed(0)}%</span></div></TooltipTrigger>{remainingDays !== null && (<TooltipContent><p>{remainingDays < 0 ? `${Math.abs(remainingDays)} days overdue` : `${remainingDays} days remaining`}</p></TooltipContent>)}</Tooltip></TooltipProvider><div className="text-right w-24"><p className="text-xs text-muted-foreground">Remaining</p><p className="font-bold text-base">${remainingAmount > 0 ? remainingAmount.toLocaleString() : 0}</p></div></div></div>)}) : <p className="text-muted-foreground text-sm py-4 text-center">No goals set. Create one in the 'Goals' tab to see progress here.</p>}</CardContent>
                        ...
                        <div className="lg:col-span-1 space-y-6">
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-3">{forecast.upcomingTransactions.length > 0 ? forecast.upcomingTransactions.map((t, i) => (<div key={i} className="flex justify-between items-center text-sm"><div><p className="font-bold">{t.description}</p><p className="text-xs text-muted-foreground">Due in {differenceInDays(t.date, new Date())} days ({format(t.date, 'MMM dd')})</p></div><p className={`font-bold ${t.type === 'earning' ? 'text-green-600' : 'text-red-600'}`}>{t.type === 'earning' ? '+' : '-'}${t.amount.toFixed(2)}</p></div>)) : <p className="text-sm text-muted-foreground text-center py-8">No upcoming recurring transactions.</p>}</CardContent>
                            </Card>
                        </div>
                    </div>
                </TabsContent>
