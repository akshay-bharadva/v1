    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button onClick={handleCreateNote}>
          <PlusCircle className="mr-2 size-4" /> Create New Note
        </Button>
      </div>
      ...
      {error && (
        <div className="rounded-md border border-destructive/50 bg-destructive/10 p-4 font-medium text-destructive">
          {error}
        </div>
      )}
      ...
      {!isLoading && notes.length === 0 ? (
        <div className="rounded-lg border-2 border-dashed border-border bg-card py-20 text-center">
          ...
          <p className="text-muted-foreground">
            Click "Create New Note" to start.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <AnimatePresence>
            {notes.map((note) => (
              ...
              >
                <Card
                  className={`flex h-full flex-col ${note.is_pinned ? "bg-secondary/50 border-accent/50" : ""}`}
                >
                  <CardHeader>
                    {note.title && (
                        ...
                        <div className="flex items-center">
                          <Tooltip>
                            <TooltipTrigger asChild>
                              ...
                              >
                                {note.is_pinned ? (
                                  <PinOff className="size-4 text-accent" />
                                ) : (
