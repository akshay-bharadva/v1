...
/*
This component is updated to use the new "glassmorphism" aesthetic.
- The timeline cards now use `bg-glass` for a floating appearance.
- The central timeline bar is given a subtle gradient for more visual interest.
- The timeline dots are enhanced with a glowing effect on hover.
*/
  ...
  return (
    <section className="my-16 py-16">
      ...
      >
        ...
        <div className="mx-auto mt-4 h-1.5 w-24 bg-gradient-to-r from-accent to-fuchsia-500" />
        <span className="absolute -top-8 left-1/2 -translate-x-1/2 text-[6rem] font-black text-white/5 -z-10 select-none" aria-hidden="true">
          CAREER
        </span>
      </motion.div>
      ...
      <div className="relative mx-auto max-w-5xl px-4">
        ...
        <div className="space-y-12">
          {MY_EXPERIENCES.map((exp, index) => (
            ...
            >
              <div className="absolute left-1/2 top-1/2 z-10 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border-4 border-background bg-border transition-all duration-300 group-hover:bg-accent group-hover:scale-125 group-hover:shadow-[0_0_20px_theme(colors.accent)]" />
              <div
                className={`w-[calc(50%-2rem)] ${index % 2 === 0 ? 'mr-auto text-right' : 'ml-auto text-left'}`}
              >
                <div className="rounded-lg bg-glass p-6 shadow-lg transition-all duration-300 group-hover:border-accent/50 group-hover:shadow-accent/10 group-hover:-translate-y-1">
